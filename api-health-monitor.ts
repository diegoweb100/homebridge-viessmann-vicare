...
export class APIHealthMonitor {
  ...
  private calculateHealthScore(): number {
    ...
    const recentSuccesses = recentRequests.filter(timestamp => {
      return timestamp <= this.metrics.lastSuccessfulRequest;
    }).length;
    
    const recentSuccessRate = recentSuccesses / recentRequests.length;
  ...
  public getDetailedStatus(): {
    ...
    if ((now - metrics.lastSuccessfulRequest) > (10 * 60 * 1000)) {
